...
class BoardModel(QAbstractListModel):
    """One row per playable hole (73 rows), with all render-time flags."""
    ...
    def __init__(self, board: Board, parent: QObject | None = None) -> None:
        ...
        self._holes: list[_Hole] = [
            _Hole(
                ...
                unitX=p.unit_xy()[0],
                unitY=p.unit_xy()[1],
                campKey=p.camps[0].value if p.camps else "",
            )
...
class PieceModel(QAbstractListModel):
    ...
    def set_players(self, players: tuple[PlayerSpec, ...]) -> None:
        """Re-seat the model for a new match (different count, colours or order)."""
...
class PlayerModel(QAbstractListModel):
    """One row per seat, in turn order.

    Rows carry both live progress (``homeCount``) and the final standing
    (``place``), so the side panel can show a running scoreboard that turns
    into the result table once the match ends.
    """
    ...
    def __init__(
        ...
        self._rows = self._build_rows(players, agent_names)

    def _build_rows(
        self, players: tuple[PlayerSpec, ...], agent_names: dict[int, str]
    ) -> list[dict[str, Any]]:
        return [
            {
                "playerId": p.id,
                ...
                "homeCount": 0,
                "campSize": len(self._board.camp_positions(p.target_camp)),
                "hasFinished": False,
                "turnIndex": index + 1,
                "place": 0,
                "placeLabel": "",
    ...
    def update(
        ...
        for row, spec in zip(self._rows, self._specs):
            ...
            place = finish_order.index(spec.id) + 1 if spec.id in finish_order else 0
            row["isCurrent"] = spec.id == current_player_id
            row["homeCount"] = home
            row["hasFinished"] = home == len(target)
            row["place"] = place
            row["placeLabel"] = _PLACE_LABELS.get(place, "")
        if self._rows:
...
class BoardGeometry(QObject):
    ...
    def __init__(
        ...
        self._camps: list[dict] = []
        self.set_players(players)

    def set_players(self, players: tuple[PlayerSpec, ...]) -> None:
        """Recolour the camp fills for a new seat layout.

        Only the colours change; the lattice itself is fixed, so bounds, edges
        and holes are computed once in ``__init__`` and never rebuilt.
        """
        board = self._board
        # Target camps first, home camps second: in a 2-player match both seats
        # share one axis, so each camp is claimed twice.  Letting the home camp
        # win means a camp is always painted in the colour of the pieces that
        # start there, instead of whichever seat was assigned last.
        colors: dict[Camp, str] = {spec.target_camp: spec.color for spec in players}
        colors.update({spec.camp: spec.color for spec in players})

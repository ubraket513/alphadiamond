...
def validate_value_target(value_target: tuple[float, ...], player_count: int) -> None:
    expected_values = 1 if player_count == 2 else 3
    if len(value_target) != expected_values:
        raise ReplayCompatibilityError("value target shape does not match Soo/Min semantics")
    if any(
        isinstance(value, bool)
        or not isinstance(value, (int, float))
        or not math.isfinite(value)
        for value in value_target
    ):
        raise ReplayCompatibilityError("value target semantics require finite numeric values")
    if player_count == 2 and value_target not in ((-1.0,), (1.0,)):
        raise ReplayCompatibilityError("Soo value target semantics require exactly -1 or +1")
    if player_count == 3 and sorted(value_target) != [-1.0, 0.0, 1.0]:
        raise ReplayCompatibilityError(
            "Min value target semantics require a permutation of (+1, 0, -1)"
        )
...
class TrainingSample:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            len(self.canonical_player_ids) != player_count
            or len(set(self.canonical_player_ids)) != player_count
        ):
            raise ReplayCompatibilityError("canonical player identity is malformed")
        validate_value_target(self.value_target, player_count)
        ...
        if any(not math.isfinite(p) or p <= 0 for p in probabilities):
            raise ReplayCompatibilityError(
                "sparse policy probabilities must be finite and positive"
            )

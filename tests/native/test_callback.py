...
def test_a_real_evaluator_locks_lanes_together_without_exploration() -> None:
    """Why stochastic MCTS had to exist before native self-play could.

    A real model callback has no per-lane salt -- it is a pure function of the
    position -- so with ``epsilon = 0`` and ``temperature = 0`` every lane from
    one opening computes the same move and N lanes are one game replayed N
    times.  Measured on the GPU host at 32 lanes, 1 trajectory.

    Throughput does not notice (a dense forward is indifferent to equal rows,
    and the evaluator is ~99 % of wall), which is what makes it dangerous: it is
    invisible in every speed number.  This pins the *cause*, so that the
    diversity test below is known to be testing exploration rather than some
    incidental source of divergence.
    """
    result = _harness().run(
        _constant,
        games=16,
    ...
    assert len(trajectories) == 1, (
        f"{len(trajectories)} distinct games from 16 lanes with no exploration"
    )


def test_exploration_makes_lanes_diverge_under_a_real_evaluator() -> None:
    """Gate E's payoff, measured through the callback rather than in isolation.

    Same evaluator, same opening, same everything except the two exploration
    knobs.  This is the property a native self-play pool needs and could not
    have before: N lanes must be N games, not N copies of one.
    """
    result = _harness().run(
        _constant,
        games=16,
    ...
    assert len(trajectories) == 16, (
        f"only {len(trajectories)} distinct games from 16 exploring lanes"
    )


def test_a_scheduled_run_is_reproducible_from_its_seed() -> None:
    """Stochastic must not mean unreproducible: same seed, same 16 games."""
    def once():
        result = _harness().run(
            _constant,
            games=16,

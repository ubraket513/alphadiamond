...
void atomic_write(const std::filesystem::path& path, const std::string& contents) {
    ...
    { std::ofstream file(temporary, std::ios::binary|std::ios::trunc); if(!file) throw std::runtime_error("cannot write replay transaction"); file<<contents; if(!file) throw std::runtime_error("cannot write replay transaction"); }
    if (std::getenv("DIAMOND_REPLAY_FAIL_ACTIVATE")) { std::filesystem::remove(temporary); throw std::runtime_error("injected replay activation failure"); }
#ifdef _WIN32
    if (!MoveFileExW(temporary.wstring().c_str(), path.wstring().c_str(), MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH)) { std::filesystem::remove(temporary); throw std::runtime_error("cannot activate replay transaction"); }
#else
...
struct ReplayStore::Impl {
    ...
    void write_manifest() {
        ...
        if (rng_algorithm == "python-mt19937") { Array words; for (auto word : mt_state) words.emplace_back(JsonValue{int64_t(word)}); words.emplace_back(JsonValue{int64_t(mt_index)}); JsonValue gauss = mt_gauss_next ? JsonValue{*mt_gauss_next} : JsonValue{nullptr}; rng["state"] = JsonValue{Array{JsonValue{int64_t(3)}, JsonValue{std::move(words)}, std::move(gauss)}}; }
        ...
        Object final{{"aborted",JsonValue{aborted_records}}, {"capacity",JsonValue{int64_t(capacity)}}, {"chunks",JsonValue{std::move(chunks)}}, {"compatibility",std::move(compat)}, {"game_ids",JsonValue{std::move(game_ids)}}, {"rng",JsonValue{std::move(rng)}}, {"schema_version",JsonValue{int64_t(2)}}};
    ...
    if (!std::filesystem::exists(impl_->manifest_path) && root_exists) {
        ...
        if (std::filesystem::exists(v2_family)) for (const auto& entry : std::filesystem::directory_iterator(v2_family, std::filesystem::directory_options::skip_permission_denied)) {
            ...
            try {
                ...
                if (integer(field(manifest, "schema_version"), "schema") == 2 && same_json(field(manifest, "compatibility"), compatibility_json(impl_->compatibility))) {

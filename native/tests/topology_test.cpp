// The topology tables are generated by the core (src/topology_gen.cpp) and can
// also be loaded from exported files.  This test checks the loader refuses
// malformed input, that the loaded tables satisfy the invariants the rest of
// the engine assumes, and -- the gate that matters -- that what the core
// generates equals the frozen exported tables entry for entry.  Those files
// came from the Python board, so this is where the two constructions are held
// to one contract.
#include <cstddef>
#include <string>
#include <vector>
...
int main(int argc, char** argv) {
    ...
    // The gate: generating the geometry must reproduce the frozen export.
    // A difference here is not a rounding error -- position ids, camp order and
    // the canonical rotation are the game's vocabulary, and a board that
    // numbers its holes differently is a different game.
    {
        const soo::Topology generated = soo::generate_topology();
        CHECK(generated.configured);
        for (int position = 0; position < soo::kBoardSize; ++position) {
            for (int direction = 0; direction < soo::kDirections; ++direction) {
                CHECK_EQ(static_cast<int>(generated.neighbour[position][direction]),
                         static_cast<int>(topology.neighbour[position][direction]));
            }
            for (int target = 0; target < soo::kBoardSize; ++target) {
                CHECK_EQ(static_cast<int>(generated.pairwise[position][target]),
                         static_cast<int>(topology.pairwise[position][target]));
            }
        }
        for (int camp = 0; camp < soo::kCamps; ++camp) {
            for (int index = 0; index < soo::kCampSize; ++index) {
                CHECK_EQ(static_cast<int>(generated.camp_positions[camp][index]),
                         static_cast<int>(topology.camp_positions[camp][index]));
            }
            for (int position = 0; position < soo::kBoardSize; ++position) {
                CHECK_EQ(static_cast<int>(generated.physical_to_canonical[camp][position]),
                         static_cast<int>(topology.physical_to_canonical[camp][position]));
                CHECK_EQ(static_cast<int>(generated.canonical_to_physical[camp][position]),
                         static_cast<int>(topology.canonical_to_physical[camp][position]));
            }
        }
    }

...
int run(const std::filesystem::path& executable, const std::filesystem::path& protocol,
    ...
    const char* arguments[] = {executable_text.c_str(), "--protocol", protocol_text.c_str(),
                               "--events-dir", events_text.c_str(), "--output",
                               output_text.c_str(), nullptr};
    ...
    return std::system(("\"" + executable.string() + "\" --protocol \"" + protocol.string() +
                        "\" --events-dir \"" + events.string() + "\" --output \"" + output.string() + "\"").c_str());
#endif
}
const Object& object(const Json& value) { return std::get<Object>(value.value); }
const Json& field(const Object& value, const char* name) { return value.at(name); }
}

int main(int argc, char** argv) {
    const auto root = std::filesystem::temp_directory_path() /
        ("alphadiamond-rating-sync-" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));
    try {
        require(argc == 2, "rating_sync_test needs sync executable path");
        using namespace diamond_orchestration;
        const auto candidate = make_participant_identity(Json{Object{{"checkpoint", Json{"candidate"}}}}, "Candidate");
        const auto baseline = make_participant_identity(Json{Object{{"checkpoint", Json{"baseline"}}}}, "Baseline");
        RatingRegistry protocol_registry{"sha256:rating-sync", EloConfig{1200.0, 24.0, 400.0, "soo-elo-v1"}};
        protocol_registry.add_participant(candidate);
        save_rating_registry(root / "protocol.json", protocol_registry);
        const auto event = make_soo_rating_event(12, "sha256:rating-sync",
            ...
            {candidate, baseline});
        write(root / "events" / (event.event_id.substr(7) + ".json"), diamond_support::canonical_json(event.to_json()));
        ...
        require(std::get<int64_t>(field(root_object, "schema_version").value) == 2, "output schema v2");
        require(std::get<int64_t>(field(object(field(root_object, "event_set")), "count").value) == 1, "output event count");
        ...
        write(root / "events" / "wrong-name.json", diamond_support::canonical_json(event.to_json()));

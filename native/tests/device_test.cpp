...
int main(int argc, char** argv) {
    ...
    if (argc == 4) {
        REQUIRE(torch::cuda::is_available(),
                "CUDA-labelled test requires an available CUDA device");
    ...
    for (const auto* malformed :
         {"CPU", "cuda:", "cuda:-1", "cuda:1x", "gpu", "cuda:999999999999999999999"})
    ...
    if (!torch::cuda::is_available()) {
        ...
        const std::string command =
            quote(training_executable) + " train --runtime-dir " + quote(runtime_dir) +
            " --model Soo --run-id unavailable-cuda --config " + quote(config) + " --checkpoint " +
            quote(scratch / "missing.pt") + " > " + quote(report) + " 2>&1";
#ifdef _WIN32

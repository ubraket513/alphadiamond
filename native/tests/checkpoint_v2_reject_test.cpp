...
int main(int argc, char** argv) {
    ...
    CHECK(rejects([&] { (void)diamond_training::inspect_checkpoint_v2(scratch / "legacy.pt"); }));

    const auto invalid_pointer = scratch / "invalid-pointer";
    write_file(invalid_pointer / "CURRENT", "../escape\n");
    CHECK(rejects([&] { (void)diamond_training::inspect_checkpoint_v2(invalid_pointer); }));
    ...
    CHECK(rejects([&] { (void)diamond_training::inspect_checkpoint_v2(incomplete); }));
    ...
    CHECK(rejects([&] { (void)diamond_training::inspect_checkpoint_v2(invalid_step); }));
    ...
    CHECK(rejects([&] { (void)diamond_training::validate_checkpoint_v2(unreadable); }));

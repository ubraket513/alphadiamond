...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < 12; ++i)
        balanced_jobs.push_back({*min_opening, static_cast<uint64_t>(100 + i)});
    ...
    const auto balanced =
        soo::run_episodes(min_match, balanced_jobs, balanced_config, evaluator, balanced_metrics);
    for (size_t i = 0; i < balanced.size(); ++i) {
        const auto& ordered = *balanced_jobs[i].match;
        std::vector<uint8_t> order;
        for (int seat = 0; seat < ordered.count; ++seat)
            order.push_back(ordered.players[seat].id);
        ...
        for (const auto& move : balanced[i].moves) {
            ...
            const auto action =
                soo::to_physical_action(move.selected_action, ordered, state.current_player);
        ...
        soo::EpisodeMetrics alone_metrics;
        const auto alone =
            soo::run_episodes(ordered, {{balanced_jobs[i].initial_state, balanced_jobs[i].seed}},

...
std::filesystem::path default_root() {
    ...
    return std::filesystem::path((local.isEmpty() ? QDir::tempPath() : local).toStdString()) /
           "ratings";
}
...
std::optional<ModelMetadata> model_metadata(const QString& modelPath, const QString& modelId) {
    ...
    if (id.size() != 2 || id[0].isEmpty() || id[1].isEmpty())
        return std::nullopt;
    ...
    if (!digest.match(result.model_digest).hasMatch() ||
        !digest.match(result.runtime_digest).hasMatch())
        return std::nullopt;
    return result;
}
...
QString sync_binary() {
    ...
    const QStringList candidates = {app_dir.filePath(name),
                                    app_dir.filePath(QStringLiteral("../") + name),
...
QString shell_program() {
    ...
    if (QFileInfo{git_bash}.isExecutable())
        return git_bash;
#endif
    return {};
}
...
QString RatingBridge::installationId() {
    ...
    if (installation_id_.isEmpty()) {
        installation_id_ =
            normalized_uuid(settings.value(QStringLiteral("ratings/installation_id")).toString());
    }
    if (installation_id_.isEmpty())
...
bool RatingBridge::writeProtocol(const QString& modelId, const QString& modelLabel,
    ...
    const QStringList id = modelId.split(QLatin1Char('/'));
    if (id.size() != 2 || id[0].isEmpty() || id[1].isEmpty() || modelLabel.isEmpty())
        return false;
...
bool RatingBridge::recordTerminalSooMatch(const QString& gameId, const QString& modelPath,
                                          ...
                                          int aiSeat, int winnerSeat,
                                          const QVariantList& turnOrder) {
    ...
    if (!metadata ||
        !writeProtocol(modelId, modelLabel, metadata->model_digest, metadata->runtime_digest)) {
    ...
    const std::array<std::string, 2> participants =
        aiSeat == 1 ? std::array{model.participant_id, human.participant_id}
...
int RatingBridge::pendingEventCount() const {
    try {
        return static_cast<int>(
            diamond_orchestration::RatingEventOutbox{root_ / "outbox"}.pending_events().size());
    } catch (const std::exception&) {
        return 0;
    }
}

bool RatingBridge::syncRunning() const {
    return sync_process_ && sync_process_->state() != QProcess::NotRunning;
}

void RatingBridge::syncPending() {
    if (syncRunning() || pendingEventCount() == 0)
        return;
    ...
    connect(sync_process_.get(), &QProcess::finished, this,
    ...
    const QString bucket =
        qEnvironmentVariable("DIAMOND_RATING_BUCKET", QStringLiteral("ubraket513/AlphaDiamond"));
    ...
    sync_process_->start(shell, {script, "--bucket", bucket, "--outbox", outbox, "--protocol",
                                 protocol, "--binary", binary});
}

void RatingBridge::setSyncState(const QString& state) {
    if (sync_state_ == state)
        return;
    sync_state_ = state;
    Q_EMIT changed();
}

...
struct ModelCatalogTestAccess {
  static void insertArtifact(ModelCatalog &catalog, const QString &id,
  ...
  static bool parseRatings(ModelCatalog &catalog, const QByteArray &payload,
                           bool persist) {
...
QString payload(const QString &modelDigest, const QString &runtimeDigest,
                double elo) {
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    ModelCatalogTestAccess::insertArtifact(
        catalog, QStringLiteral("soo/2.0.0"), QStringLiteral("2.0.0"),
        modelDigest, runtimeDigest);

    require(ModelCatalogTestAccess::parseRatings(
                catalog,
                payload(modelDigest, runtimeDigest, 1203.5).toUtf8(), false),
            "matching ratings JSON is accepted");
    require(catalog.models().front().toMap()
                    .value(QStringLiteral("latestElo"))
                    .toString() == QStringLiteral("1203.50"),
            "rating matches full artifact identity");
    require(ModelCatalogTestAccess::parseRatings(
                catalog,
                payload(QString(64, QLatin1Char('c')), runtimeDigest, 9999.0)
                    .toUtf8(),
                false),
    ...
    require(!ModelCatalogTestAccess::parseRatings(
                catalog, QByteArrayLiteral("{}"), false),
